    ...
    for package in "${@}"; do
    	...
    	if check_pkg "${package}"; then
            ...
            if test "${?}" -eq 1; then
                ...
                notify-send "${package:u} INSTALLED SUCCESSFULLY" --icon="${f[alfred]}"
...
deemix_stuffs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[1]}[[:space:]]/ {print }") ]]; then
        ...
        if [[ ! -e "${f[decrypt]}" ]]; then
            ...
            wget --quiet "${l[2]}" --output-document "${f[decrypt]}"
...
usefull_pkgs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[1]}[[:space:]]/ {print }") \
        ...
        if [[ ! -e "${f[series]}" ]]; then
            ...
            [[ ! -d "${d[4]}" || $(stat -c "%U" "${d[4]}" 2>&-) != "${USER}" ]] \
                && sudo mkdir --parents "${d[4]}" > "${f[null]}" \
                && sudo chown --recursive "${USER}":"${USER}" "${d[4]}"

            show "\n${c[YELLOW]}${m[17]:u} ${c[WHITE]}${linen:${#m[17]}} [INSTALLING]"

            wget --quiet "${l[2]}" --output-document "${f[rar-file]}"

            sudo mkdir --parents "${d[5]}" > "${f[null]}"

            tar --extract --gzip --file="${f[rar-file]}" --directory="${d[5]}" > "${f[null]}"
...
change_panelandgui() {
    ...
    f+=(
        ...
        [bkg-grubtheme]=/boot/grub/themes/linuxmint-2k/background.jpg
    ...
    [[ ! -e "${f[alfred]}" ]] \
        && curl --location --output "${f[alfred]}" --create-dirs "${l[6]}"  # END ICON
    ...
    [[ ! -e "${f[bkg-grubtheme]}" ]] \
        && wget --quiet "${l[5]}" --output-document "${f[bkg-grubtheme]}"
