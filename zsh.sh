...
alexa_stuffs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[1]}[[:space:]]/ {print }") ]]; then
        ...
        install_packages "${m[3]}" "${m[5]}"

        # lib m[5] always broken for being so old
        sudo apt update &> "${f[null]}"
...
deemix_stuffs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[1]}[[:space:]]/ {print }") \
        ...
        show "${c[GREEN]}\n        I${c[WHITE]}NSTALLING ${c[GREEN]}${m[23]:u}${c[WHITE]} AND ${c[GREEN]}DEPENDENCIES${c[WHITE]}!" 'fast'
    ...
    [[ ! -e "${f[clear_garbage]}" ]] \
        && sudo tee "${f[clear_garbage]}" > "${f[null]}" <<< "#!/usr/bin/env zsh
...
rename 's/^(Dj|dj|mc|Mc)/\U$1/' ${XDG_MUSIC_DIR}/* && rename 's|Ac_dc|ACDC|g' ${XDG_MUSIC_DIR}/* && find ${XDG_MUSIC_DIR}/ -type f -regex '.*\.mp3\|.*\.flac\|.*\.lrc' -exec rename 's/ \(.*\)//' {} \;"
    ...
    sudo sed --in-place 's|"maxBitrate":.*|"maxBitrate": "9",|g' "${f[cfg]}"
...
docky_stuffs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[6]}[[:space:]]/ {print }") ]]; then
        ...
        show "${c[GREEN]}\n\t  I${c[WHITE]}NSTALLING ${c[GREEN]}${m[6]:u}${c[WHITE]} AND ${c[GREEN]}DEPENDENCIES${c[WHITE]}!" 'fast'
...
dualmonitor_stuffs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[2]}[[:space:]]/ {print }") \
        ...
        show "${c[GREEN]}\n\t  S${c[WHITE]}ETING ${c[GREEN]}${m[1]:u}${c[WHITE]} AND ${c[GREEN]}CONFIGURATING${c[WHITE]}!" 'fast'
    ...
    for (( ; ; )); do
        ...
        if [[ "${option:0:1}" =~ ^(s|S|y|Y)$ ]] ; then
            ...
            show "${c[GREEN]}\n       I${c[WHITE]}NSTALLING ${c[GREEN]}${m[5]:u}${c[WHITE]} AND ${c[GREEN]}DEPENDENCIES${c[WHITE]}!" 'fast'
    ...
    if ( $(dpkg --compare-versions "${local}" lt "${latest}") ); then
        ...
        for (( ; ; )); do
            ...
            if [[ "${option:0:1}" =~ ^(s|S|y|Y)$ ]] ; then
                ...
                pip install --quiet --no-warn-script-location "${m[3]}"=="${latest}"
...
flameshot_stuffs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[1]}[[:space:]]/ {print }") ]]; then
        ...
        show "${c[GREEN]}\n\tI${c[WHITE]}NSTALLING ${c[GREEN]}${m[1]:u}${c[WHITE]} AND ${c[GREEN]}CONFIGURATING${c[WHITE]}!" 'fast'
...
python_stuffs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[1]}[[:space:]]/ {print }") \
        ...
        show "\n${c[GREEN]}${m[1]:u} ${c[WHITE]}${linei:${#m[1]}} [INSTALLED]" 'fast'
    ...
    if ( $(dpkg --compare-versions "${local}" lt "${latest}") ); then
        ...
        for (( ; ; )); do
            ...
            if [[ "${option:0:1}" =~ ^(s|S|y|Y)$ ]] ; then
                ...
                show "${c[GREEN]}\n\t   I${c[WHITE]}NSTALLING ${c[GREEN]}${m[4]:u}${c[WHITE]} AND ${c[GREEN]}DEPENDENCIES${c[WHITE]}!" 'fast'
...
sublime_stuffs() {
    ...
    declare -a l=(
        ...
        'https://www.sublimetext.com/download'  # 5
    )
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[2]}[[:space:]]/ {print }") ]]; then
        ...
        show "${c[GREEN]}\n       I${c[WHITE]}NSTALLING ${c[GREEN]}${m[2]:u}${c[WHITE]} AND ${c[GREEN]}DEPENDENCIES${c[WHITE]}!" 'fast'

        # 2> hides
        # Warning: apt-key output should not be parsed (stdout is not a terminal)
        [[ ! $(sudo apt-key list 2> "${f[null]}" | grep Sublime) ]] \
            && sudo wget --quiet --output-document - "${l[1]}" | gpg --dearmor | sudo tee "${f[gpg]}" &> "${f[null]}"
    ...
    latest=$(curl --silent "${l[5]}" | grep latest | awk '{print $4}' | sed 's/<[^>]*>//g')

    if ( $(dpkg --compare-versions "${local}" lt "${latest}") ); then

        echo; read $'?\033[1;37mSIR, SHOULD I UPGRADE SUBLIME TEXT VERSION FROM '${local}' TO '${latest}$'? \n[Y/N] R: \033[m' option
        ...
        for (( ; ; )); do
            ...
            if [[ "${option:0:1}" =~ ^(s|S|y|Y)$ ]] ; then
                ...
                update && sudo apt install --assume-yes "${m[2]}" &> "${f[null]}"
...
usefull_pkgs() {
    ...
    if [[ $(dpkg --list | awk "/ii  ${m[1]}[[:space:]]/ {print }") \
        ...
        show "${c[GREEN]}\n     I${c[WHITE]}NSTALLING ${c[GREEN]}${m[7]:u}${c[WHITE]} AND ${c[GREEN]}CONFIGURATING${c[WHITE]}!" 'fast'
...
change_panelandgui() {
    ...
    local -a l=(
        ...
        'https://gist.githubusercontent.com/rafaelribeiroo/a7e8b342308f4381c5fc4798624be9b0/raw/711322654b44b1507f6ae66607a7cb91dfbc21e7/manage_files.sh'  # 2
    )
    ...
    local -a m=(
        ...
        'update-notifier-common'  # 9
        'language-pack-pt-base'  # 10
        'language-pack-gnome-pt-base'  # 11
        'hyphen-fi'  # 12
        'hyphen-ga'  # 13
        'hyphen-id'  # 14
    )

    install_packages "${m[1]}" "${m[2]}" "${m[10]}" "${m[11]}" "${m[12]}" "${m[13]}" "${m[14]}"
    ...
    for (( ; ; )); do
        ...
        if [[ "${option:0:1}" =~ ^(s|S|y|Y)$ ]] ; then
            ...
            [[ ! -e "${f[util_extract]}" ]] \
                && sudo wget --quiet "${l[2]}" --output-document "${f[util_extract]}"
...
@reboot zsh ${f[util_extract]}" \
    ...
    if [[ ! $(grep --no-messages 'Update-Package' "${f[auto_upgrade]}") ]]; then
        ...
        install_packages "${m[8]}" "${m[9]}"

        echo; read $'?\033[1;37mENTER YOUR EMAIL, '"${name[random]}"$': \033[m' email
...
menu() {
    ...
    for (( ; ; )); do
        ...
        for line in "${(@k)logo}"; do
            ...
            show "    ${c[RED]}${(P)logo}${line}" 'fast' && sleep 0.1s
        ...
        sleep 0.1s; show "${c[RED]}[ 02 ] ${c[WHITE]}BRAVE BROWSER ${e[leo]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 03 ] ${c[WHITE]}DEEMIX ${e[headphone]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 04 ] ${c[WHITE]}DOCKY ${e[control]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 05 ] ${c[WHITE]}DUAL MONITOR SETUP ${e[landscape]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 06 ] ${c[WHITE]}GIT/GITHUB ${e[octopus]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 07 ] ${c[WHITE]}GOOGLE CHROME ${e[globe]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 08 ] ${c[WHITE]}FLAMESHOT ${e[camera]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 09 ] ${c[WHITE]}HEROKU ${e[rocket]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 10 ] ${c[WHITE]}HIDE WINDOWS DEVICES (DUAL BOOT) ${e[blind_monkey]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 11 ] ${c[WHITE]}MINIDLNA ${e[popcorn]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 12 ] ${c[WHITE]}NVIDIA DRIVER ${e[n]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 13 ] ${c[WHITE]}POSTGRES ${e[elephant]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 14 ] ${c[WHITE]}POSTMAN ${e[satellite]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 15 ] ${c[WHITE]}PYTHON ${e[snake]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 16 ] ${c[WHITE]}REDUCE EYE STRAIN ${e[moon]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 17 ] ${c[WHITE]}RUBY ${e[ruby]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 18 ] ${c[WHITE]}SUBLIME TEXT ${e[letters]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 19 ] ${c[WHITE]}TMATE ${e[magnet]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 20 ] ${c[WHITE]}USEFULL PROGRAMS ${e[diamond]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 21 ] ${c[WHITE]}WORKSPACE ${e[suitcase]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 22 ] ${c[WHITE]}XSCREENSAVER ${e[screensaver]}" 'fast'
        sleep 0.1s; show "${c[RED]}[ 23 ] ${c[WHITE]}ZSH (OH-MY-ZSH) ${e[paint]}" 'fast'
