...
read -p $'\033[1;37mSIR, WANT TO INSTALL MORE PACKAGES? \n[Y/N] R: \033[m' option

for (( ; ; )); do

    if [[ "${option:0:1}" = @(s|S|y|Y) ]] ; then

        echo; read -p $'\033[1;37mPACKAGE TITLE (CASE SENSITIVE): \033[m' pkg

        [[ $(curl --write-out %{http_code} --silent --output "${f[null]}" "${l[0]}""${pkg}") -ne 200 ]] \
        ...
        if ! [[ "${pkg}" =~ ^(Anaconda|Djaneiro|Restart|SublimeREPL|Sublimerge Pro|Dracula Color Scheme)$ ]]; then
            ...
            echo; show "\t\t  REPO ALREADY ${c[RED]}DOWNLOADED${c[WHITE]}"
